(function () {
  ...
  function valRecaptcha(errorElementId, widgetId) {
    ...
    try {
      ...
      response =
        widgetId !== undefined && widgetId !== null
  ...
  function valName(el, optional) {
    ...
    if (v.length < 2) {
      showError(el, "Name must be at least 2 characters.");
      return false;
    }
    if (v.length > 60) {
      showError(el, "Name must be under 60 characters.");
      return false;
    }
    showValid(el);
    return true;
  ...
  function valPhone(el) {
    ...
    if (!/^[0-9]{10}$/.test(v)) {
      showError(el, "Phone must be exactly 10 digits.");
      return false;
    }
    showValid(el);
    return true;
  ...
  function valSubject(el) {
    var v = el.value.trim();
    if (!v) {
      showError(el, "Subject is required.");
      return false;
    }
    if (v.length < 3) {
      showError(el, "Subject must be at least 3 characters.");
      return false;
    }
    if (v.length > 100) {
      showError(el, "Subject must be under 100 characters.");
      return false;
    }
    showValid(el);
    return true;
  ...
  function valMessage(el, min, max) {
    var v = el.value.trim();
    if (!v) {
      showError(el, "Message is required.");
      return false;
    }
    if (v.length < min) {
      showError(el, "Message must be at least " + min + " characters.");
      return false;
    }
    if (v.length > max) {
      showError(el, "Message must be under " + max + " characters.");
      return false;
    }
    showValid(el);
    return true;
  ...
  function valCareerName(el) {
    var v = el.value.trim();
    if (!v) {
      showError(el, "Career name is required.");
      return false;
    }
    if (v.length < 2) {
      showError(el, "Career name must be at least 2 characters.");
      return false;
    }
    if (v.length > 80) {
      showError(el, "Career name must be under 80 characters.");
      return false;
    }
    showValid(el);
    return true;
  ...
  function valReason(el) {
    var v = el.value.trim();
    if (!v) {
      showError(el, "Please tell us why this career is hatke.");
      return false;
    }
    if (v.length < 10) {
      showError(el, "Please write at least 10 characters.");
      return false;
    }
    if (v.length > 500) {
      showError(el, "Keep it under 500 characters.");
      return false;
    }
    showValid(el);
    return true;
  ...
  function initSuggestion() {
    ...
    if (career) {
      career.addEventListener("blur", function () {
        valCareerName(career);
      });
      liveClear(career);
    }
    if (reason) {
    ...
    if (suggester) {
      suggester.addEventListener("blur", function () {
        if (suggester.value.trim()) valName(suggester, true);
      });
      liveClear(suggester);
    }
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (suggester && suggester.value.trim() && !valName(suggester, true))
        ok = false;
